"""
A copy of
MESSENGER_Region_Detection/application/visualisation/application_viewer.py to
show a specific ideal example for the paper
"""
...
# print(f"Start: {start}")
# print(f"End: {end}")
messenger_data = mag.Load_Between_Dates(utils.User.DATA_DIRECTORIES["MAG"], start, end)
...
for ax in axes:
    ...
    if zoom_in == "right":
        ax.xaxis.set_major_locator(
            matplotlib.dates.MinuteLocator(byminute=np.arange(0, 60, 10))
        )
        ax.set_xlim(dt.datetime(2011, 4, 3, 17, 20), ax.get_xlim()[1])

    elif zoom_in == "left":
        ax.xaxis.set_major_locator(
            matplotlib.dates.MinuteLocator(byminute=np.arange(0, 60, 5))
        )
        ax.set_xlim(ax.get_xlim()[0], dt.datetime(2011, 4, 3, 16, 10))
...
if zoom_in == "right":
    plt.savefig("./figures/fig09_ideal_application_example_bs_zoom.pdf", format="pdf")

elif zoom_in == "left":
    plt.savefig("./figures/ideal_application_example_mp_zoom.pdf", format="pdf")

else:
    plt.savefig("./figures/ideal_application_example.pdf", format="pdf")
